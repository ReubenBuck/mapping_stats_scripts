...
# we look at the content of bins that align before we start doing it at different scales 

# make sure to put the functions in the work space

# get this into an image prducing pipeline, that way we can prepare all the data and actually look at how things in the unscalled and untrasformaed genomes actaully look. 

# so we've looked to see if starting conditions change things
# likly not the case 
# potential bug in codes

# how does it work as a sample of the genome
# 

# maybe do this with and without sex chromosomes for several speceies 
# save each species in a package type thing then show results on laptop. 
...
pdf(file = paste("~/Desktop/mapping_stats_analysis/plots/bar_line_plot/", spec2,"_stats_",chr_main,".pdf", sep = ""),onefile = TRUE)
...
pdf(file = paste("~/Desktop/mapping_stats_analysis/plots/pca_plot/", spec2,"_biplot_",chr_main,"_no_scale",".pdf", sep = ""),onefile = TRUE)
...
pdf(file = paste("~/Desktop/mapping_stats_analysis/plots/pca_plot/", spec2,"_biplot_",chr_main,"_scaled",".pdf", sep = ""),onefile = TRUE)
...
for(c in 1:length(cutoff)){
	...
	ggsave(filename = paste("~/Desktop/mapping_stats_analysis/plots/karyogram/",spec2,"/", spec1, "_karyogram_compared_to_",spec2,"_at_cutoff_", cutoff[c],"_for_", chr_main,".pdf", sep = ""))
}
...
for(c in 1:length(cutoff)){
	...
	ggsave(filename = paste("~/Desktop/mapping_stats_analysis/plots/karyogram/", spec2,"/", spec2,"_karyogram_at_",cutoff[c],"_for_",chr_main,".pdf", sep = ""))
...
pdf(file = paste("~/Desktop/mapping_stats_analysis/plots/heatmap/", spec2,"_heatmap_",chr_main,".pdf", sep = ""),onefile = TRUE) 
for(c in 1:length(cutoff)){
...
# spec 1 self corrolation
heatmap.2(cor(s1[,5:length(s1)], method = "spearman"), 
			scale = "none",
			col = my_palette,
			symkey = TRUE,
			breaks = seq(from = -1, to = 1, length = 20),
...
#			xlab = spec2,
#			ylab = spec1,
			main = paste(spec1,"spatial TE correlations"),
			denscol=1
#			RowSideColors = my_p_val_palette[as.integer(results.p.s1[,c] * 100) + 1],
#			ColSideColors = my_p_val_palette[as.integer(results.p.s1[,c] * 100) + 1]
			)
...
#			xlab = spec2,
#			ylab = spec1,
			main = paste(spec2, "spatial TE correlations"),
			denscol=1,
#			RowSideColors = my_p_val_palette[as.integer(results.p.s2[,c] * 100) + 1],
#			ColSideColors = my_p_val_palette[as.integer(results.p.s2[,c] * 100) + 1]
			)
...
# maybe understand the variation in correlation scores accross cutoffs, could make a heatmap out of that




# find a way to plot these as backups for funny results 
...
for(i in 1:15){
	te <- hist(TE.s2[,i])
	re <- hist(TE.replot[,i])
	plot(te$mids, te$density, type = "l")
	lines(re$mids, re$density, col = 2)
...
for(i in 1:15){
	te <- hist(TE.s2[,i])
	re <- hist(TE.replot[,i])
	plot(te$mids, te$count, type = "l")
	lines(re$mids, re$count, col = 2)
	
}

i = i +1

d.te.s <- density(s2_replot_0.05[,i])
d.te.r <- density(TE.replot[,i])
plot(d.te.s)
lines(d.te.r,col = 2)

# also whcih bins are we comparing across, 

# making an assumption of what a humans TE content would be if it had 



# so leets make a series of dnsity plots
...
for(c in 1:4){
d.te.r <- density(bovine[,i])
d.te.s <- density(get(paste("s2_replot_", cutoff[c], sep = ""))[,i])
plot(d.te.r, main = "", xlab = "elements per bin")
lines(d.te.s,col = 2)

text(x = max(d.te.r$x)*.75, y = max(d.te.r$y)*.9, labels = paste("cutoff =",cutoff[c]), cex = .8)
text(x = max(d.te.r$x)*.75, y = max(d.te.r$y)*.8, labels = paste("ks.test p-value ~",round(results.p.s2[i,c], digits = 2)), cex = .8)

}

plot(NA)
legend(.5,.5, fill = c(1,2), c("original dog genome", "remodeled dog genome"))





#### so now we need to read the unalignable section of the genome 
## what kind of regions are we missing
# the idea might be that these are centromeric regions or telomeric regions and are probably worth getting rid of.
## this way we look at just important genic components. 

# at each cut off stage we do some images 
# the pca, and the karyogram 
# i could potentially do that today



# karyogram looks interesting, douple check that it is not poorly sequenced regions 
# it may be that the white gaps are regions that don't align to cow


# really we could change this function to colour in certain bins 


# so it does seem that the white sections are just poorly sequenced
# I also should get the actuall chromosome lengths so that I can draw proper karyograms

# maybe if we move away from the rates and just 
# focus on bins with the right amoutn of known bases we can get a case for better mapping
# and we also focus particularly on the autosomes
# we may be able to represent a good section of the genome when remapping. 
# not removing regions thst don't map well but removing regions that are lilky to have poorly annotated repeats
# pooprly annotated because large parts of them were not sequenced correctly 


# might be worth using mappings that occur in the other direction.

# if we do that we probably have to change the S1 S2 names, see what happens when we align cow to human 
# another idea is to merge the datasets 


# so we can only do this with a limited phylogenetic distance



~/Desktop/mapping_stats_analysis/plots/bar_line_plot
