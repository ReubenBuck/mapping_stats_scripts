...
#s1.comp <- data.frame(scale(s1.comp, center = T))
#s2.comp <- data.frame(scale(s2.comp, center = T))
...
t1 <- qplot(s1.comp$new_LINE, s2.comp$new_LINE, col = d.te.n) + scale_color_gradient(low = "lightblue", high = "red") + labs(title = paste(spec2, colnames(s1.comp)[1])) + guides(col = FALSE)
t2 <- qplot(s1.comp$new_SINE, s2.comp$new_SINE, col = d.te.n) + scale_color_gradient(low = "lightblue", high = "red") + labs(title = paste(spec2, colnames(s1.comp)[2])) + guides(col = FALSE)
t3 <- qplot(s1.comp$MIR, s2.comp$MIR, col = d.te.n) + scale_color_gradient(low = "lightblue", high = "red") + labs(title = paste(spec2, colnames(s1.comp)[3])) + guides(col = FALSE)
t4 <- qplot(s1.comp$L2, s2.comp$L2, col = d.te.n) + scale_color_gradient(low = "lightblue", high = "red")+ labs(title = paste(spec2, colnames(s1.comp)[4])) + guides(col = FALSE)
...
for(i in 1:4){
	...
	assign(
		paste("p", i , sep =""), 
		ggplot(dat, aes(x = S1, y = S2, col = groups)) +  geom_point() + labs(title = paste(spec2, colnames(s1.comp)[i])) + guides(col = FALSE) + xlim(min(dat$S1), max(dat$S1)) + ylim(min(dat$S2), max(dat$S2))
	)
	
	assign(
		paste("q", i , sep =""),
		ggplot(dat, aes(x = S1, y = S2, col = groups) ) + geom_density2d()  + labs(title = paste(spec2, colnames(s1.comp)[i])) + guides(col = FALSE) + xlim(min(dat$S1), max(dat$S1)) + ylim(min(dat$S2), max(dat$S2))	
	)
		
	assign(
		paste("r", i , sep =""),
		ggplot(dat, aes(x = S1, y = S2, col = groups) ) + geom_density2d()  + labs(title = paste(spec2, colnames(s1.comp)[i])) + guides(col = FALSE) + xlim(min(dat$S1), max(dat$S1)) + ylim(min(dat$S2), max(dat$S2))	 + geom_point()
	)
...
# is there a cooler way to show these results? # we probably should get it so we can either put the lines in or go one dimensional
